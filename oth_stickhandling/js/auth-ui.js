...
(function () {
  ...
  /* ── Sync mobile overlay auth section ───────────────────────── */
  function syncMobileAuth(mode) {
    const mobStatus  = document.getElementById('mob-auth-status');
    const mobActions = document.getElementById('mob-auth-actions');
    if (!mobStatus || !mobActions) return;
    ...
    if (mode === 'auth') {
      ...
      const pin = A().getPin();
      mobStatus.innerHTML = `Code: <strong>${pin}</strong>`;
      mobActions.innerHTML =
        `<button class="mob-auth-btn mob-auth-btn-ghost" onclick="OTHAuthUI.showPinEntry();closeMenu()">Change code</button>` +
        `<button class="mob-auth-btn mob-auth-btn-primary" onclick="OTHAuthUI.showSignUp();closeMenu()">Create account</button>`;

    } else {
      mobStatus.textContent = 'Save your progress';
      mobActions.innerHTML =
        `<button class="mob-auth-btn mob-auth-btn-ghost" onclick="OTHAuthUI.getCode();closeMenu()">Get a code</button>` +
        `<button class="mob-auth-btn mob-auth-btn-ghost" onclick="OTHAuthUI.showPinEntry();closeMenu()">Enter code</button>` +
        `<button class="mob-auth-btn mob-auth-btn-primary" onclick="OTHAuthUI.showSignIn();closeMenu()">Sign in</button>`;
    }
  }
