...
(function () {
  ...
  function renderBar(mode) {
    ...
    if (mode === 'auth') {
      statusEl.textContent = '✓ Signed in — progress synced across devices';
  ...
  function btn(label, onclick, primary = false) {
    ...
    return `<button onclick="${onclick}" style="
      ...
      font-family:inherit; white-space:nowrap; font-weight:500">${label}</button>`;
  }

  /* ── Reload progress + rebuild after identity change ─────────── */
  ...
  async function getCode() {
    ...
    modal.open(`
      ...
      <div style="
        ...
        letter-spacing:.14em; text-align:center; border-radius:10px;
        padding:1.1rem; margin-bottom:1rem; user-select:all">
        ${pin}
      </div>
      ...
      <p style="text-align:center;margin-top:1rem;color:#aaa;font-size:.82rem">
        ...
        <span onclick="OTHAuthUI.showSignUp()" style="color:#e04050;cursor:pointer">
  ...
  function showPinEntry() {
    modal.open(`
      ...
      <input id="oth-pin-input" type="text" placeholder="OTH-XXXX"
        ...
        style="${inputStyle} text-transform:uppercase; letter-spacing:.1em;
      ...
      <p style="text-align:center;color:#aaa;font-size:.82rem;margin:0">
        Don't have a code?
        <span onclick="OTHAuthUI.getCode()" style="color:#e04050;cursor:pointer">
          Get one now
        </span>
      </p>
    `);
    setTimeout(() => {
      ...
      if (inp) {
        ...
        inp.addEventListener('keydown', e => {
          if (e.key === 'Enter') window.OTHAuthUI.submitPin();
        });
      }
    }, 50);
  }
  ...
  function showSignUp() {
    modal.open(`
      ...
      <input id="oth-su-pass" type="password" placeholder="Password (min 6 characters)"
  ...
  async function submitSignUp() {
    ...
    modal.open(`
      ...
      <button onclick="OTHAuthUI.showSignIn()" style="${primaryBtnStyle}">
        Go to Sign In
      </button>
    `);
  }
  ...
  function showSignIn() {
    modal.open(`
      ...
      <button onclick="OTHAuthUI.submitSignIn()" style="${primaryBtnStyle} margin-bottom:.75rem">
        Sign In
      </button>
      <p style="text-align:center;color:#aaa;font-size:.82rem;margin:0 0 .5rem">
        <span onclick="OTHAuthUI.showForgotPassword()" style="color:#e04050;cursor:pointer">
          Forgot password?
        </span>
      </p>
      <p style="text-align:center;color:#aaa;font-size:.82rem;margin:0">
    ...
    setTimeout(() => {
      ...
      if (passInp) {
        passInp.addEventListener('keydown', e => {
          if (e.key === 'Enter') window.OTHAuthUI.submitSignIn();
        });
      }
    }, 50);
  }
  ...
  function showForgotPassword() {
    modal.open(`
      ...
      <input id="oth-fp-email" type="email" placeholder="Email"
        autocomplete="email" style="${inputStyle}" />
      <div id="oth-fp-err" style="color:#e04050;font-size:.85rem;min-height:1.2rem;margin-bottom:.5rem"></div>
      <button onclick="OTHAuthUI.submitForgotPassword()" style="${primaryBtnStyle} margin-bottom:.75rem">
        Send Reset Link
      </button>
      <p style="text-align:center;color:#aaa;font-size:.82rem;margin:0">
        <span onclick="OTHAuthUI.showSignIn()" style="color:#e04050;cursor:pointer">
          ← Back to sign in
        </span>
      </p>
    `);
    setTimeout(() => {
      ...
      if (inp) {
        inp.focus();
        inp.addEventListener('keydown', e => {
          if (e.key === 'Enter') window.OTHAuthUI.submitForgotPassword();
        });
      }
    }, 50);
  }
  ...
  async function submitForgotPassword() {
    ...
    modal.open(`
      ...
      <p style="color:#aaa;line-height:1.6;margin:0 0 1.5rem">
        Check your inbox at <strong style="color:#fff">${email}</strong> for a
        password reset link.
      </p>
